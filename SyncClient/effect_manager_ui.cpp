...
void EffectManagerUI::loadEffectParameters(const QString &effectName)
{
    ...
    for (const auto &[key, param] : params)
    {
        ...
        if (std::holds_alternative<int>(param))
        {
            ...
            spin->setValue(std::get<int>(param));
            input = spin;
        }
        // FLOAT
        else if (std::holds_alternative<float>(param))
        {
            ...
            dspin->setValue(std::get<float>(param));
            input = dspin;
        }
        // STRING
        else if (std::holds_alternative<std::string>(param))
        {
            auto *line = new QLineEdit(QString::fromStdString(std::get<std::string>(param)));
            input = line;
        }
        // COLOR
        else if (std::holds_alternative<std::array<uint8_t, 3>>(param))
        {
            QPushButton *colorButton = new QPushButton("Pick Color");
            std::array<uint8_t, 3> color = std::get<std::array<uint8_t, 3>>(param);
