...
EffectManagerUI::EffectManagerUI(QWidget *parent) : QMainWindow(parent)
{
    ...
    connect(effectList, &QComboBox::currentTextChanged, this, &EffectManagerUI::loadEffectParameters);
    connect(startButton, &QPushButton::clicked, this, &EffectManagerUI::startEffect);
    connect(stopButton, &QPushButton::clicked, this, &EffectManagerUI::stopEffect);
...
void EffectManagerUI::loadEffectParameters(const QString &effectName)
{
    ...
    for (const auto &[key, param] : params)
    {
        ...
        {
            auto *line = new QLineEdit(QString::fromStdString(std::get<std::string>(param.value)));
            input = line;
        }
        // COLOR
        else if (std::holds_alternative<std::array<uint8_t, 3>>(param.value))
        {
            ...
            std::array<uint8_t, 3> color = std::get<std::array<uint8_t, 3>>(param.value);

            QObject::connect(colorButton, &QPushButton::clicked, [colorButton, &color]()
                             {
                QColor chosen = QColorDialog::getColor(QColor(color[0], color[1], color[2]));
...
void EffectManagerUI::applyEffectParameters(std::string effect_name)
{
    ...
    for (const auto &[key, widget] : inputWidgets)
    {
        ...
        {
            LOGGER.debug("Setting string parameter {}", line->text().toStdString());
        ...
        else if (auto *colorButton = qobject_cast<QPushButton *>(widget))
        {
            ...
            LOGGER.debug("Setting color parameter ({}, {}, {})", color[0], color[1], color[2]);
            params[key] = color;
        }
    }
